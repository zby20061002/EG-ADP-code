...
FINAL_MODEL_DIR = "./final_models_fair"
...
for fold in range(FOLDS):
    ...
    # 训练每个方法的专用模型
    models = {}  # method_name -> trained model
    for method_name, apply_fn in protect_methods.items():
        if method_name == 'Original':
            # 仅原始图像
            print(f"训练 {method_name} 模型 (仅原始)...")
            model = train_emotion_model(train_orig, val_orig, class_weights)
        else:
            # 原始 + 保护图像
            train_prot = load_samples(train_df, apply_fn)
            val_prot = load_samples(val_df, apply_fn)
            train_mixed = train_orig + train_prot
            val_mixed = val_orig + val_prot
            print(f"训练 {method_name} 模型 (原始 + 保护)...")
            model = train_emotion_model(train_mixed, val_mixed, class_weights)
        models[method_name] = model
    
    # 评估：每个模型测试对应保护图像（以及原始图像，可选）
    test_versions = list(protect_methods.keys())  # 包括 Original
    for model_method, model in models.items():
        # 该模型测试它自己的保护图像版本
        test_prot_fn = protect_methods[model_method]
        test_samples_eval = load_samples(test_df, test_prot_fn)
        test_samples_id = load_samples(test_df, test_prot_fn, return_subject=True)
        
        # 情感指标
        acc, macro_f1, f1_sad, f1_fear, f1_dis = evaluate_emotion_model(model, test_samples_eval)
        # 身份指标 (FaceNet & ArcFace)
        top1_fn, auc_fn, eer_fn = evaluate_identity(test_samples_id, gallery_facenet, 'facenet')
        top1_af, auc_af, eer_af = evaluate_identity(test_samples_id, gallery_arcface, 'arcface')
        
        key = f"{model_method}_Fair"
        results[key].append({
        ...
        print(f"  {model_method} Fair: Acc={acc:.4f}, Macro-F1={macro_f1:.4f}, SadF1={f1_sad:.4f}, FN_Top1={top1_fn:.4f}, AF_Top1={top1_af:.4f}")

# ==================== 汇总 ====================
print("\n" + "="*80)
print("5-fold Fair Baseline 结果")
print("="*80)
table_rows = []
...
df_final.to_csv("5fold_fair_baseline_results.csv", index=False)
print("\n结果已保存至 5fold_fair_baseline_results.csv")