import os, sys, cv2, torch, numpy as np, pandas as pd, random
...
EMOTION_MODEL_PATH = r"F:\python\2024218729zby_ML\best_emotion_ckplus.pth"
...
# ==================== 加载 InSwapper ====================
print("加载 InSwapper...")
SWAPPER_MODEL = r"F:\python\models\insightface\models\chwshuang\inswapper_128.onnx\inswapper_128.onnx"
assert os.path.exists(SWAPPER_MODEL), f"InSwapper 模型未找到: {SWAPPER_MODEL}"
swapper = get_model(SWAPPER_MODEL, download=False)
# InSwapper 不需要 prepare，直接可用
print("InSwapper 已就绪。")
...
def evaluate_identity(probe_embs, probe_subjects, gallery_embs):
    ...
    for emb, subj in zip(probe_embs, probe_subjects):
        if subj not in gallery_embs: continue
...
def semantic_laplace(img_rgb, sigma_emo, sigma_id, sigma_nc):
    ...
    result = (emo_mask[...,None]*(img_f+noise_emo) + id_mask[...,None]*(img_f+noise_id) + nc_mask[...,None]*(img_f+noise_nc))
    return np.clip(result, 0, 255).astype(np.uint8)

def semantic_laplace_high(img_rgb):
    return semantic_laplace(img_rgb, sigma_emo=8, sigma_id=25, sigma_nc=45)

# ---- InSwapper ----
def build_anon_library(df_test):
    lib = []
    seen = set()
...
def inswapper_anonymize(img_rgb):
    global ANON_LIBRARY
    if not ANON_LIBRARY:
        return img_rgb
    ...
    if not faces:
        return img_rgb
    source_face = random.choice(ANON_LIBRARY)
    result_bgr = swapper.get(img_bgr, faces[0], source_face, paste_back=True)
...
protect_methods = {
    ...
    'Strong Mosaic': lambda img: cv2.resize(cv2.resize(img, (img.shape[1]//16, img.shape[0]//16), interpolation=cv2.INTER_LINEAR),
                                            (img.shape[1], img.shape[0]), interpolation=cv2.INTER_NEAREST),
    'EG-ADP': apply_ours,
    'Pixel-Laplace-σ10': lambda img: laplace_noise(img, sigma=10),
    'Semantic-Laplace-high': semantic_laplace_high,
    'InSwapper': inswapper_anonymize,
}
...
ANON_LIBRARY = build_anon_library(df_test)
print(f"匿名库构建完成，包含 {len(ANON_LIBRARY)} 个身份。")
    ...
    for protocol, data_dict, gallery_dict in [
        ...
        for model_name in ['facenet','arcface','extra']:
            ...
            print(f"{method_name:20s} | {protocol:20s} | {model_name:8s} | "
                  f"Acc={acc:.4f} MacroF1={macro_f1:.4f} DetTop1={top1:.4f} OverallTop1={overall_top1:.4f}")

df_res = pd.DataFrame(summary_rows)
df_res.to_csv("final_complete_baselines.csv", index=False)
print("\n所有实验完成，结果保存至 final_complete_baselines.csv")