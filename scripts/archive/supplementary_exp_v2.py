...
EMOTION_MODEL_PATH = "best_emotion_ckplus.pth"   # 混合训练模型，请确认文件名
...
for method_name, protect_fn in protect_methods.items():
    print(f"\n--- {method_name} ---")
    # 存储 probe 数据，统一使用 (embeddings, subjects) 对
    det_data = {'facenet': ([], []), 'arcface': ([], [], []), 'extra': ([], [], [])}  # 第三个列表保存检测标志
    ...
    for protocol, data_dict, gallery_dict in [
        ...
        for model in ['facenet','arcface','extra']:
            ...
            print(f"{method_name:12s} | {protocol:20s} | {model:8s} | "
                  f"Det-only Top1={top1:.4f} | Overall Top1={overall_top1:.4f} | "
