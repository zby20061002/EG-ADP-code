...
def get_region_masks(parsing, emo_dilation=3):
    """返回表情区、眼鼻区、非核心区的 0-1 掩膜，表情区膨胀尺寸可调"""
    ...
    id_mask = cv2.dilate(id_mask, np.ones((3,3),np.uint8), iterations=1)   # 眼鼻保持小膨胀
...
def apply_region_processing(img_rgb, parsing, proc_emo, proc_id, proc_nc, emo_dilation=3):
    emo_mask, id_mask, nc_mask = get_region_masks(parsing, emo_dilation)
...
# ==================== 定义待测配置 ====================
# 格式：名称, 表情noise (0表示保留), 眼鼻blur ksize, 非核心mosaic block, 表情膨胀kernel size
configs = [
    ("A1_emo0_id13_nc12",  0.0,  13, 12, 3),
    ("A2_emo0_id15_nc12",  0.0,  15, 12, 3),
    ("A3_emo0_id17_nc12",  0.0,  17, 12, 3),
    ("A4_emo0_id19_nc12",  0.0,  19, 12, 3),
    ("A5_emo0_id15_nc14",  0.0,  15, 14, 3),
    ("A6_emo0_id17_nc14",  0.0,  17, 14, 3),
    ("A7_emo001_id15_nc14",0.001,15, 14, 3),
    ("A8_emo001_id17_nc14",0.001,17, 14, 3),
    # 额外膨胀实验，以 A5 为基准变 dilation
    ("A5_d7",  0.0,  15, 14, 7),
    ("A5_d11", 0.0,  15, 14, 11),
]

results = []
for name, emo_noise, id_blur, nc_mosaic, emo_dil in configs:
    print(f"\n--- 测试 {name} ---")
    # 构建处理器
    proc_emo = keep_original if emo_noise == 0 else noise_processor(emo_noise)
    proc_id = blur_processor(id_blur)
    proc_nc = mosaic_processor(nc_mosaic)
    ...
    for subj, true_label, img_rgb in tqdm(probe_data, desc=name):
        ...
        protected = apply_region_processing(img_rgb, parsing, proc_emo, proc_id, proc_nc, emo_dil)
...
df_res.to_csv("fine_tune_results.csv", index=False)
print("\n结果已保存至 fine_tune_results.csv")